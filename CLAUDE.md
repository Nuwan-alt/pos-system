...
- `server/middleware/verifyAdminPassword.js` — validates `adminPassword` in request body against `settings` table
...
- `server/utils/passwordEncoding.js` — `encodePassword(plain)`, base64-encodes a password; used by every route/middleware that reads or writes `admin_password`/`cashier_password` so the `settings` table never holds plain text
...
- **Cash drawer** — one record per day (`drawer_date UNIQUE`); open/close/reset/edit; admin-only reset/edit; polls every 30s
- **Stock updates** — audit-logged; admin uses "adjust", cashier uses "update"; both atomic, both signed quantity deltas
